...
/**
 * Build script for the DSH Model Search Plugin.
 *
...
// The compiled ESM uses `import { … } from './types.js'` and `export { … }`.
// We need to replace imports with a require() call and wrap in the factory.
//
// Strategy: detect import lines and emit them as require() inside the factory,
// then wrap the remaining code.

const importPattern = /^import\s+(?:(?:\*\s+as\s+(\w+))|(?:\{([^}]+)\})|(\w+))\s+from\s+['"]([^'"]+)['"]\s*;?\s*$/gm;

// Inline the types module since it's small and self-contained.
// For the client.js source we resolve ./types.js imports.
...
let factoryCode = compiledCode
  // Remove all import statements (we'll inline the types module)
  .replace(/^import\s+.*?from\s+['"][^'"]+['"]\s*;?\s*$/gm, '')
  // Replace `export { ... }` at the end of the file
  ...
  // Replace `export function` / `export const` / `export class`
  .replace(/^export\s+(function|const|class|let|var|interface|type)\s+/gm, '$1 ')
  // Replace `export default` 
  .replace(/^export\s+default\s+/gm, '')
  // Remove TypeScript type annotations that survived compilation
  .replace(/: (\w+)(?:\[\])?(?:\s*=\s*)/g, (match, type) => {
    // Keep simple types -> remove
    if (['string', 'number', 'boolean', 'void', 'any', 'null', 'undefined', 'never', 'unknown'].includes(type)) {
      return ' ';
    }
    return match;
  })
  // Remove type imports (they compile to nothing in ESM)
  .replace(/^import\s+type\s+.*$/gm, '');

// Clean up empty lines and extra whitespace from import removal
...
const pluginId = 'dsh-model-search-plugin';
...
const bundleContent = `window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ── inlined @dsh-model-search-plugin/types ─────────────────────
${TYPES_CODE.split('\n').map(l => '    ' + l).join('\n')}
...
if (!existsSync(LIB_INDEX)) {
  // If tsc didn't produce it, create a minimal one
  const fallback = `export function apply(ctx) { /* dsh-model-search-plugin host stub */ }\n`;
...
writeFileSync(resolve(DIST_DIR, 'dsh-model-search-plugin.js'), standaloneDist, 'utf-8');
console.log('[build] Wrote', resolve(DIST_DIR, 'dsh-model-search-plugin.js'));
