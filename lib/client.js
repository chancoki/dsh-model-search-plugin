window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function onSearchInput(ev) {
        const el = ev?.target ?? this ?? state.searchEl ?? pickerState.searchEl;
        if (!el)
            return;
    ...
    function onSearchKeydown(e) {
        ...
        if (e.key === 'ArrowDown') {
            const firstOption = pickerState.listEl?.querySelector('li:not([style*="none"]) label') ??
                state.groupsEl?.querySelector('[role="menuitemradio"]');
            firstOption?.focus();
            e.preventDefault();
        }
    }
    ...
    function onGlobalKeydown(e) {
        if (!config.focusHotkey)
            return;
        if (e.key !== config.focusHotkey)
            return;
        if (config.focusRequiresModifier && !(e.metaKey || e.ctrlKey))
            return;
        if (!state.isActive)
            return;
    ...
    function injectSearchIntoPicker(listEl) {
        // Don't inject twice
        if (pickerState.searchEl && listEl.contains(pickerState.searchEl))
            return;
        const input = buildSearchUI(PICKER_PLACEHOLDER);
        const container = input.closest(`.${config.containerClass}`);
    ...
    function detachFromPicker() {
        ...
        container.forEach((el) => {
            ...
            if (!el.contains(state.searchEl ?? null))
                el.remove();
        });
        if (pickerDebounceTimer) {
            clearTimeout(pickerDebounceTimer);
    ...
    function performSearch(query) {
        ...
        if (q.length < config.minQueryLength) {
            // Show all
            const allOptions = groupsEl.querySelectorAll('[role="menuitemradio"]');
            for (const opt of allOptions) {
                opt.style.display = '';
                opt.classList.remove('model-search-match');
            }
            const allSections = groupsEl.querySelectorAll('section[role="group"]');
        ...
        let totalMatches = 0;
        const sections = groupsEl.querySelectorAll('section[role="group"]');
        for (const section of sections) {
            const options = section.querySelectorAll('[role="menuitemradio"]');
    ...
    function findGroupsContainer(menuEl) {
        ...
        const groupsWrapper = menuEl.querySelector('div:not([role])');
    ...
    function injectSearchIntoMenu(menuEl, groupsEl) {
        // Don't inject twice
        if (state.searchEl && menuEl.contains(state.searchEl))
            return;
        ...
        state.documentObserver = new MutationObserver((mutations) => {
            for (const mutation of mutations) {
                for (const node of mutation.addedNodes) {
                    ...
                    const pickerList = (node.matches?.('ul[class*="candidateList"]') ? node : null) ??
                        node.querySelector('ul[class*="candidateList"]');
                    if (pickerList) {
                    ...
                    const menu = node.querySelector('[role="menu"][aria-label*="model" i],[role="menu"][aria-label*="推理等级"],[role="menu"][aria-label*="effort" i]');
                    if (menu) {
                        state.menuEl = menu;
